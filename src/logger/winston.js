...
const rabbitMqLogger = winston.createLogger({
    level: "info",
    format: combine(
        timestamp({ format: "YYYY-MM-DD HH:mm:ss" }),
        logFormat
    ...
    transports: [
        new winston.transports.Console(),
        new DailyRotateFile({ filename: "var/log/mqLogs-%DATE%.log", datePattern: "YYYY-MM-DD", zippedArchive: true, maxsize: 10485760, maxFiles: "10d", level: "info", auditFile: false })
    ]
});
...
exports.info_mqLogger = async function (message) {
    if (rabbitMqFlag) {
        rabbitMqLogger.info(`${message}`)
    }
};